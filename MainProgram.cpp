...
public:
    ...
    explicit Temperature(double celsius) {
        ...
        if (celsius < - 273.15) {
        throw invalid_argument("Tempereture cannot be below absolute zero");
    }
        celsius_ = celsius;
...
public:
    ...
    BankAccount(const string& owner, double initialBalance) {
        ...
        if (initialBalance < 0){
            throw invalid_argument("Balance cannot be less than zero");
        }
        owner_ = owner;
    ...
    void deposit(double amount) {
        if (amount <= 0){
            throw invalid_argument("Deposit must be bigger than zero");
        }
...
private:
    ...
    static void validate(const string& pwd) {
        if (pwd.length() < 8){
            throw invalid_argument("Your password must be at least 8 characters long");
        }
        if (!hasDigit(pwd)) {
             throw invalid_argument("Your password must contain at least one digit");
        }
